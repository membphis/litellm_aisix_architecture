...
这些问题需要与大量用户沟通、经历生产环境事件迭代后才能得出合理答案。
项目初期尚未积累足够的用户反馈，过早抽象反而会固化错误的边界。

#### 时间节点

预计 **1-2 个季度后**，随着核心功能趋于稳定、用户反馈逐步积累，
...
- 某些功能已确认高度稳定，边界清晰
- 积累了足够的生产用户反馈，对各阶段语义有明确共识
- 出现了无法通过当前 HTTP callback 扩展点满足的真实诉求

届时面临的核心选择：**局部改造**（仅对稳定阶段引入 plugin 接口）还是
**全面改造**（将整个请求链路重构为可配置的 stage pipeline）——
这个问题目前刻意保持开放，待生产经验积累后再做决策。
