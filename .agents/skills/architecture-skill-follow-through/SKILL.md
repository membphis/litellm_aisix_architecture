...
| 4. Stabilize | Analyze/coordinate changed skill set (no file edits): identify related files, check overlaps/conflicts, split unrelated changes | Internal checkpoint, not a user gate. If goals changed → back to Step 2. **Semantic contradictions in related skill docs must be classified as must-do, not optional — skill layer consistency is non-negotiable.** |
...
| 7. Handoff | Invoke `writing-plans` with resolved Summary 2 as input | After writing-plans produces the plan document, **trace every item from Summary 2** (must-do, optional, deferred) to a task in the plan. Any item without a corresponding task must have an explicit defer reason recorded in the plan. Fix gaps before announcing plan complete. Skill terminates when writing-plans completes. |
