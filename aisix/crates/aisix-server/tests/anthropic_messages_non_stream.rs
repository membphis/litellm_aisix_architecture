...
fn anthropic_non_stream_request() -> Request<Body> {
...
fn anthropic_request_without_version() -> Request<Body> {
    Request::builder()
        .method("POST")
        .uri("/v1/messages")
...
fn anthropic_request_with_tools() -> Request<Body> {
    anthropic_request(json!({
        "model": "gpt-4o-mini",
        "messages": [{"role": "user", "content": "hello"}],
        "max_tokens": 256,
...
fn anthropic_request_with_bearer() -> Request<Body> {
    Request::builder()
        .method("POST")
        .uri("/v1/messages")
        .header("content-type", "application/json")
