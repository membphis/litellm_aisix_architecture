...
impl EtcdHarness {
    ...
    pub async fn get_json(&self, key: &str) -> Result<Option<serde_json::Value>> {
        ...
        let response = client
            .get(key, None)
            .await
            .context("failed to get etcd fixture")?;
        let Some(kv) = response.kvs().first() else {
            return Ok(None);
        };

        let value = serde_json::from_slice(kv.value()).context("failed to decode etcd fixture")?;
        Ok(Some(value))
    }
    ...
    pub async fn compact(&self, revision: i64) -> Result<()> {
        ...
        client
            .compact(
                revision,
                Some(etcd_client::CompactionOptions::new().with_physical()),
            )
            .await
            .context("failed to compact etcd fixture")?;
